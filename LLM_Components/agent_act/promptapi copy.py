...
class APISearcher:
    ...
    def parse_gpt_response(self, response_content: str) -> List[Tuple[str, str]]:
        ...
        for line in lines:
            match = re.match(r'torch\.(\S+)', line)
            if match:
                function_name = match.group(1).strip()
                suggestions.append(('torch', function_name))
        return suggestions
    ...
    def search_api(self, prompt: str) -> List[Tuple[str, Any]]:
        ...
        response = self.client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a Python expert assistant specializing in PyTorch."},
                {"role": "user", "content": f"Based on the prompt '{prompt}', suggest potential PyTorch functions, classes, or methods that might be relevant for image classification. Format your response as a list of 'torch.function_or_class' items, one per line. Focus only on PyTorch (torch module) and its submodules."},
            ],
        )
        ...
        torch_module = self.load_module('torch')
        if torch_module:
            for _, function_name in suggestions:
                attr = torch_module
                for part in function_name.split('.'):
                    if hasattr(attr, part):
                        ...
                        attr = None
                        logging.warning(f"'{function_name}' not found in Torch module.")
                        break
                if attr:
                    results.append((f"torch.{function_name}", attr))
        return results
