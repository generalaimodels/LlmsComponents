...
def test_ffmpeg_read(audio_file_path):
    print("Testing ffmpeg_read:")
    try:
        with open(audio_file_path, "rb") as audio_file:
            audio_data = audio_file.read()
        
        sampling_rate = 16000  # Adjust this to match your audio file's sampling rate
        audio = ffmpeg_read(audio_data, sampling_rate)
        
        print(f"Successfully read audio file. Shape: {audio.shape}, dtype: {audio.dtype}")
        print(f"First 10 samples: {audio[:10]}")
    except Exception as e:
        print(f"Error in ffmpeg_read: {str(e)}")
    print()

def test_ffmpeg_microphone():
    print("Testing ffmpeg_microphone:")
    try:
        sampling_rate = 16000
        chunk_length_s = 30
        
        microphone = ffmpeg_microphone(sampling_rate, chunk_length_s)
        
        print("Recording for 3 seconds...")
        audio_chunk = next(microphone)
        
        audio = np.frombuffer(audio_chunk, dtype=np.float32)
        print(f"Successfully recorded audio. Shape: {audio.shape}, dtype: {audio.dtype}")
        print(f"First 10 samples: {audio[:10]}")
    except Exception as e:
        print(f"Error in ffmpeg_microphone: {str(e)}")
    print()

def test_ffmpeg_microphone_live():
    print("Testing ffmpeg_microphone_live:")
    try:
        sampling_rate = 16000
        chunk_length_s = 3
        stream_chunk_s = 1
        
        stream = ffmpeg_microphone_live(sampling_rate, chunk_length_s, stream_chunk_s)
        ...
        while time.time() - start_time < 10:
            ...
            print(f"  Shape: {chunk['raw'].shape}, dtype: {chunk['raw'].dtype}")
            print(f"  Partial: {chunk['partial']}")
            print(f"  Stride: {chunk['stride']}")
            print(f"  First 5 samples: {chunk['raw'][:5]}")
            print()
        
        print(f"Received {chunk_count} chunks in total.")
    except Exception as e:
