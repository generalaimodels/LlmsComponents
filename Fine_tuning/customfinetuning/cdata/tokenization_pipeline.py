    ...
    def __init__(self, tokenizer: PreTrainedTokenizer) -> None:
        ...
        'target': ['Greeting', 'Statement', 'Description']
         ...
         try:
             ...
             logger.info(f"Sample input_ids: {result['input_ids'][0][:10]}...")
             logger.info(f"Sample attention_mask: {result['attention_mask'][0][:10]}...")
             logger.info(f"Sample labels: {result['labels'][0][:10]}...")
             logger.info(f"Sample label_g: {result['label_g'][0][:10]}...")
